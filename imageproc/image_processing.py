...
def brightenIntensity(intensity):
    red_inten = numpy.int32(intensity[0]) + 10
    if red_inten > 255:
        red_inten = 255
    green_inten = numpy.int32(intensity[1]) + 10
    if green_inten > 255:
        green_inten = 255
    blue_inten = numpy.int32(intensity[2]) + 10
    if blue_inten > 255:
        blue_inten = 255

    return ((numpy.int8(red_inten),numpy.int8(green_inten), numpy.int8(blue_inten)))

        
def darkenIntensity(intensity):
    red_in = numpy.int32(intensity[0]) - 10
    if red_in < 0:
        red_in = 0
    green_in = numpy.int32(intensity[1]) - 10
    if green_in < 0:
        green_in = 0
    blue_in = numpy.int32(intensity[2]) - 10
    if blue_in < 0:
        blue_in = 0

    return ((numpy.int8(red_in), numpy.int8(green_in),numpy.int8 (blue_in)))
...
def lighten(image_surf):
    ...
    for x in range(rows):
        for y in range(cols):
            pixels3d[x,y] = brightenIntensity(pixels3d[x,y])
...
def darken(image_surf):
    ...
    for x in range(rows):
        for y in range(cols):
            pixels3d[x,y] = darkenIntensity(pixels3d[x,y])
